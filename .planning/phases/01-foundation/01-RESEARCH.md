...
- `packages/web/tsconfig.json` — strict, bundler module resolution, paths for `@/*` and `@agentforge/shared/*` [VERIFIED: codebase grep]
...
go mod init github.com/agentforge/worker
...
pnpm --filter @agentforge/shared install

# Vitest for web package
pnpm --filter @agentforge/web add -D vitest @vitejs/plugin-react jsdom @testing-library/react @testing-library/dom vite-tsconfig-paths
```
...
│       └── package.json        # name: @agentforge/shared
├── supabase/
...
import (
    ...
    "github.com/agentforge/worker/internal/grpc"
    "github.com/agentforge/worker/internal/http"
)
...
services:
  web:
    image: agentforge/web:${VERSION:-latest}
    build:
      context: ../packages/web
  ...
  worker:
    image: agentforge/worker:${VERSION:-latest}
    build:
      context: ../packages/worker
      dockerfile: Dockerfile
    ports:
...
| Quick run (web) | `pnpm --filter @agentforge/web test --run` |
| Full suite (web) | `pnpm --filter @agentforge/web test` |
...
|--------|----------|-----------|-------------------|-------------|
| FOUN-02 | Next.js compiles without errors | compile | `pnpm --filter @agentforge/web typecheck` | ❌ Wave 0 |
| FOUN-02 | Root layout renders | unit | `pnpm --filter @agentforge/web test --run` | ❌ Wave 0 |
...
- [ ] Vitest install: `pnpm --filter @agentforge/web add -D vitest @vitejs/plugin-react jsdom @testing-library/react @testing-library/dom vite-tsconfig-paths`

#### Sampling Rate
- **Per task commit:** `pnpm --filter @agentforge/web typecheck && pnpm --filter @agentforge/web lint`
...
**How to avoid:** Either (a) add a stub `package.json` to `packages/worker/` with just `{"name": "@agentforge/worker", "private": true}`, or (b) explicitly list only JS packages in `pnpm-workspace.yaml`.
...
1. **Shared package scope**
   - What we know: `pnpm-workspace.yaml` covers `packages/*`, and `tsconfig.json` has `@agentforge/shared/*` path alias pointing to `../shared/*`
