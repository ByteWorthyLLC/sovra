...
- **packages/web/vitest.config.ts** — jsdom environment, `@vitejs/plugin-react`, `vite-tsconfig-paths` for `@/*` and `@agentforge/shared/*` path resolution.
...
|-------|---------|--------|
| Vitest | `pnpm --filter @agentforge/web run test` | 2/2 passed |
| TypeScript | `pnpm --filter @agentforge/web run type-check` | Clean (exit 0) |
| ESLint | `pnpm --filter @agentforge/web run lint` | No warnings or errors |
